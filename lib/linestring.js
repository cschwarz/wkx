...
LineString._parseWkb = function (value, options) {
    ...
    for (var i = 0; i < pointCount; i++) {
        lineString.points.push(new Point(value.readDouble(), 
                                         value.readDouble(),
...
LineString.prototype.toWkb = function () {
    ...
    this._writeWkbType(wkb, Types.wkb.LineString);
    ...
    for (var i = 0; i < this.points.length; i++) {
        ...
        if (this.hasZ)
            wkb.writeDoubleLE(this.points[i].z);
        if (this.hasM)
            wkb.writeDoubleLE(this.points[i].m);
    }
