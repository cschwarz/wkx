...
LineString.prototype.toTwkb = function () {
    var twkb = new BinaryWriter(this._getTwkbSize());
    ...
    if (this.points.length > 0) {
        ...
        var y = 0;
        for (var i = 0; i < this.points.length; i++) {
            x = ZigZag.encode(this.points[i].x * precisionFactor) - x;
            y = ZigZag.encode(this.points[i].y * precisionFactor) - y;
...
LineString.prototype._getTwkbSize = function () {
    if (this.points.length === 0)
        return 2;
